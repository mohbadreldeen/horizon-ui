...
const Checkbox = React.forwardRef<React.ElementRef<typeof CheckboxPrimitive.Root>, React.ComponentPropsWithoutRef<typeof CheckboxPrimitive.Root>>(({ className, ...props }, ref) => {
    return (
        <CheckboxPrimitive.Root
            ref={ref}
            className={cn(
                'grid place-content-center peer h-6 w-6 shrink-0 rounded-md border-2 data-[state=checked]:border-transparent font-bold border-secondary-foreground-100 ring-offset-background focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50 data-[state=checked]:bg-link-foreground data-[state=checked]:text-white',
                className
            )}
            {...props}
        >
